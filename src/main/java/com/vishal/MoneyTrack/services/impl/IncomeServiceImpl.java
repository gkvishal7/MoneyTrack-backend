...
public class IncomeServiceImpl implements IncomeService {
    ...
    @Override
    public List<IncomeResponse> getByDateRange(LocalDate startDate, LocalDate endDate) {
        UUID userId = SecurityUtils.getCurrentUserId();
        return incomeRepository.findIncomesByDateRange(startDate, endDate, userId).stream()
                .map(mapper::toResponse)
                .toList();
    }

    @Override
    public List<IncomeResponse> getByAccountId(UUID accountId) {
        UUID userId = SecurityUtils.getCurrentUserId();
        return incomeRepository.findByAccountIdAndUserId(accountId, userId).stream()
                .map(mapper::toResponse)
                .toList();
    }
